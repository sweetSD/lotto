...
class LottoWinResultWidget extends StatelessWidget {
  ...
  const LottoWinResultWidget(this.lotto, {Key? key, this.useDecoration = true})
      : super(key: key);
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          ...
          LottoText(
            '${DateFormat('yyyy-MM-dd').format(lotto!.drawAt!)} 추첨',
            size: 10,
          ),
...
class LottoPickWidget extends StatelessWidget {
  ...
  const LottoPickWidget(this.picks,
      {Key? key,
      this.index,
      this.rank,
      this.color = const Color(0xffeeeeee),
      this.result,
      this.onlyPicks = false})
      : super(key: key);
              ...
              Row(
                children: <Widget>[
                  for (int i = 0; i < picks.length; i++) ...[
                    result != null && !result!.contains(picks[i])
                        ? LottoBall(
                            picks[i],
